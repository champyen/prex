...
### Stage 3: ELF Loader Update for XIP (Step 5.2)
...
* **r9 register setup:** During thread creation/context initialization, set `r9` to point to the task's SRAM data region.
...
* **Context switching:** Implement context switching via `cpu_switch` in `locore.S`.
