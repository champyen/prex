...
pub fn valid(task: kern.TaskRef) callconv(.c) c_int {
    ...
    while (n != @as(*hal.List, @ptrCast(&task_list))) : (n = list_next_node(n)) {
        const tmp = lib.IntrusiveList(kern.Task, hal.List, "link").parent(n);
        if (tmp == task) {
            return 1;
        }
    }
...
pub fn terminate(task: kern.TaskRef) callconv(.c) c_int {
...

    {
        ...
        while (n != @as(*hal.List, @ptrCast(&task.?.*.threads))) {
            ...
            const t = lib.IntrusiveList(kern.Thread, hal.List, "task_link").parent(n);
...
pub fn info(task_info_ptr: ?*hal.TaskInfo) callconv(.c) c_int {
    ...
    while (true) : (n = list_next_node(n)) {
        if (i == target) {
            const task = lib.IntrusiveList(kern.Task, hal.List, "link").parent(n);
